...
(define dummy-state-map (ann (hash (Posn 0 0) 0 (Posn 1 0) 1) (StateMap Posn Integer)))
(define dummy-world : (World Posn Posn Integer) (World dummy-state-map cartesian-topology (lambda (c) #t)))
...
(define (make-2d-renderer color-map #:cell-width-px [cell-width-px 30] #:origin-px [origin-px (Posn 0 0)] #:width-px [disp-width MAIN-DISPLAY-WIDTH] #:height-px [disp-height MAIN-DISPLAY-HEIGHT])
  ...
  (define (last-idx origin-px-component disp-len) (cast (ceiling (/ (+ origin-px-component disp-len) cell-width-px)) Integer))
